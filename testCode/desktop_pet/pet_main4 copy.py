...
class DesktopPet(QLabel):
    def __init__(self):
        ...
        self.target_short_side = 128  # 定义短边目标长度
    ...
    def change_skin(self, skin_name):
        ...
        self.movie.frameChanged.connect(self.update_movie_size)  # 重新绑定信号
        self.movie.start()
        QTimer.singleShot(100, self.update_movie_size)  # 确保首次加载触发
        
    def update_movie_size(self):
        """ 在 GIF 载入后，动态调整大小（固定短边为 128） """
        if self.movie is None:
            return
        ...
        if original_size.isValid() and original_size.width() > 0 and original_size.height() > 0:
            ...
            # 计算短边固定为 128 的比例
            if width <= height:  # 宽是短边或正方形
                new_width = self.target_short_side
                new_height = int((height / width) * self.target_short_side)
            else:  # 高是短边
                new_height = self.target_short_side
                new_width = int((width / height) * self.target_short_side)
