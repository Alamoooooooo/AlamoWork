...
class DesktopPet(QLabel):
    ...
    def init_ui(self):
        ...
        self.movie.frameChanged.connect(self.update_movie_size)  # 每次帧变化时调整窗口大小
    ...
    def update_movie_size(self):
        ...
        if frame_size.isValid():
            # new_width, new_height = frame_size.width(), frame_size.height()

            # if new_width > new_height:
            #     # 如果宽度更大，固定宽度为128，计算高度
            #     new_width = 128
            #     new_height = int((new_height / new_width) * 128)
            # else:
            #     # 如果高度更大，固定高度为128，计算宽度
            #     new_height = 128
            #     new_width = int((new_width / new_height) * 128)
            ...
            # 调整 QMovie 的缩放大小
            self.movie.setScaledSize(frame_size)
    ...
    def change_skin(self, skin_name):
        ...
        # 延迟调整大小，确保 GIF 已加载
        QTimer.singleShot(100, self.update_movie_size)
